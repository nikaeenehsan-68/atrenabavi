...
export class TextbooksService {
  ...
  async create(dto: CreateTextbookDto) {
    ...
    try {
      ...
      this.logger.error(`Create textbook failed: ${e?.message}`, e?.stack || '');
  ...
  async findOne(id: number) {
    ...
    if (!row) throw new NotFoundException('درس یافت نشد');
    return row;
  }
  ...
  async update(id: number, dto: UpdateTextbookDto) {
    ...
    try {
      ...
      this.logger.error(`Update textbook ${id} failed: ${e?.message}`, e?.stack || '');
      throw new InternalServerErrorException('DB error', { cause: e });
    }
  }
