...
export default function ClassesListPage() {
  ...
  const [editForm, setEditForm] = useState({ code: "", name: "", grade_level_id: 0 });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setYear(y || null);
      if (y?.id) {
        const [cls, gr] = await Promise.all([
          fetch(`/api/classes?academic_year_id=${y.id}`, { cache: "no-store" }).then(async r=>{const t=await r.text(); try{return JSON.parse(t)}catch{return []}}),
          fetch(`/api/grade-levels`, { cache: "no-store" }).then(async r=>{const t=await r.text(); try{return JSON.parse(t)}catch{return []}}),
        ]);
        setRows(Array.isArray(cls) ? cls : []);
  ...
  function openEdit(r: Row) {
    setEditTarget(r);
    setEditForm({ code: r.code || "", name: r.name || "", grade_level_id: r.grade_level_id });
  ...
  async function saveEdit(e: React.FormEvent) {
    ...
    setErr(null);
    try {
      const r = await fetch(`/api/classes/${editTarget.id}`, {
        method: "PATCH",
        ...
        body: JSON.stringify({
          ...
          // academic_year_id را تغییر نمی‌دهیم
        }),
      });
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border p-5 md:p-7" dir="rtl">
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-xl md:text-2xl font-extrabold">لیست کلاس‌ها {year?.title ? `- ${year.title}` : ""}</h1>
        <Link href="/admin/classes/new" className="px-4 py-2 rounded-xl bg-teal-600 text-white hover:bg-teal-700 shadow-sm">
          افزودن کلاس
        </Link>
      </div>
      ...
      <div className="rounded-xl border bg-white p-4 overflow-x-auto">
        {loading ? (
          ...
          <div className="text-gray-500 text-sm">کلاسی برای سال جاری ثبت نشده است.</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="text-gray-500">
                <th className="text-right py-2 border-b">کد</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  <td className="py-2 border-b">{r.code}</td>
      ...
      {editTarget && (
        <Modal onClose={() => setEditTarget(null)}>
          <div className="font-semibold mb-3">ویرایش کلاس</div>
          <form onSubmit={saveEdit} className="grid md:grid-cols-2 gap-3">
            <label className="block">
              <div className="mb-1 text-sm text-gray-600">کد</div>
              <input
      ...
      {deleteTarget && (
        <Modal onClose={() => setDeleteTarget(null)}>
          <div className="font-semibold mb-2">حذف کلاس</div>
